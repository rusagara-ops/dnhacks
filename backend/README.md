...
Mac identity migration: registration optionally accepts `previous_device_id`. When the new stable ID has no row, the previous installation's row is locked and updated to the new ID, retaining its worker ID and task history. Subsequent registrations upsert by the stable ID. Offline historical registrations are retained for attribution; the demo Compute machines cards display online machines only (including busy machines).
